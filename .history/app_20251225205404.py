...
def venues():
  ...
  for venue in venues:
    ...
    areas[key].append({
      "id": venue.id,
      "name": venue.name,
      "num_upcoming_shows": 0  # we’ll compute this later
    })
...
def create_venue_submission():
  ...
  try:
    venue = Venue(
      ...
      #genres is a string, not a list. This join concats the multiple values into a list
    ...
    #add items to change in schema and commit those changes
...
def edit_venue_submission(venue_id):
  ...
  try:
    ...
    db.session.rollback()
    flash(f'An error occurred. Artist {venue.name} could not be updated.')
...
def shows():
  ...
  for show in shows:
    ...
    "start_time": shows.start_time.strftime("%Y-%m-%dT%H:%M:%S")
  })
